#!/usr/bin/env python3
"""
Download images for 200 saltwater tropical fish species.
Uses Wikimedia Commons API with thumbnail URLs.

Usage:
    python3 download-200-saltwater.py              # Download all species
    python3 download-200-saltwater.py --start 100  # Start from item 100
"""
...
def search_wikimedia_commons(query):
    ...
    try:
        req = urllib.request.Request(
            search_url,
            headers={'User-Agent': 'SaltwaterTropicalFishImageDownloader/1.0 (educational project)'}
        )
...
def download_image(url, output_path):
    ...
    try:
        req = urllib.request.Request(
            url,
            headers={'User-Agent': 'Mozilla/5.0 (compatible; SaltwaterTropicalFishImageDownloader/1.0)'}
        )
...
def main():
    ...
    print(f"Downloading images for {total} saltwater tropical fish species...")
