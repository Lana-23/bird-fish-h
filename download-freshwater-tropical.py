#!/usr/bin/env python3
"""
Download images for tropical freshwater fish species.
Uses Wikimedia Commons API with thumbnail URLs.

Usage:
    python3 download-freshwater-tropical.py              # Download all species
    python3 download-freshwater-tropical.py --start 25   # Start from item 25
    python3 download-freshwater-tropical.py --delay 5    # Custom delay
"""
...
def search_wikimedia_commons(query):
    ...
    try:
        req = urllib.request.Request(
            search_url,
            headers={'User-Agent': 'FreshwaterTropicalFishDownloader/1.0 (educational project)'}
        )
...
def download_image(url, output_path):
    ...
    try:
        req = urllib.request.Request(
            url,
            headers={'User-Agent': 'Mozilla/5.0 (compatible; FreshwaterTropicalFishDownloader/1.0)'}
        )
...
def main():
    ...
    print("=" * 70)
    print("DOWNLOAD TROPICAL FRESHWATER FISH IMAGES")
    print("=" * 70)
    ...
    for i, (species_id, latin_name) in enumerate(species_list, start=start_index):
        print(f"[{i + 1}/{total}] {species_id} ({latin_name})")
