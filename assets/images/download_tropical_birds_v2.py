#!/usr/bin/env python3
"""
Script to download tropical bird images using multiple sources
Usage: python3 download_tropical_birds_v2.py
"""
...
def get_image_urls(sources, search_term, bird_id):
    """Try multiple image sources"""
    urls = []
    
    # Source 1: Wikimedia Commons API
    try:
        wiki_url = f"https://commons.wikimedia.org/w/api.php?action=query&format=json&list=search&srsearch={search_term.replace(' ', '+')}+bird&srprop=imageinfo&srlimit=5"
        response = requests.get(wiki_url, timeout=10, headers={'User-Agent': 'BirdTracker/1.0'})
    ...
    # Source 2: Unsplash (via source API)
    urls.append(f"https://images.unsplash.com/photo-1544605954-10244f789507?w=800")  # Generic bird fallback
    
    return urls


def download_image(bird_id, search_term=None, max_retries=2):
    ...
    sources = [
        # Wikimedia Commons
        lambda: f"https://commons.wikimedia.org/w/api.php?action=query&format=json&generator=search&gsrsearch={search_term.replace(' ', '+')}+bird&gsrlimit=3",
    ]

    for attempt in range(max_retries):
        try:
            # Try Wikimedia Commons
            wiki_search = f"https://commons.wikimedia.org/w/api.php?action=query&format=json&generator=search&gsrsearch={search_term.replace(' ', '+')}&gsrnamespace=6&gsrlimit=3"
            response = requests.get(wiki_search, timeout=15, headers={'User-Agent': 'BirdTracker/1.0 (contact: birdtracker@example.com)'})
            ...
            if response.status_code == 200:
                ...
                for page_id, page_data in pages.items():
                    ...
                    # Download the image
                    img_response = requests.get(image_url, timeout=30, headers={'User-Agent': 'BirdTracker/1.0'})
                    
                    if img_response.status_code == 200 and len(img_response.content) > 5000:
                        with open(output_path, 'wb') as f:
                            f.write(img_response.content)
                        print(f"✓ {bird_id}: Downloaded from Wikimedia")
                        time.sleep(0.3)
                        return True
            
            print(f"✗ {bird_id}: No image found (attempt {attempt + 1})")
            
        except Exception as e:
            if attempt == max_retries - 1:
                print(f"✗ {bird_id}: Failed - {str(e)[:60]}")
            time.sleep(1)
